...
MANUAL OWNER ACTION: onboard `playground.com.pl` in Cloudflare and migrate nameservers. Create a remotely managed tunnel with public hostnames `playground.com.pl` and `www.playground.com.pl` targeting `http://gateway:80`. Put its token only in `/srv/familyplaces/.env.production` mode `0600`.
