...
- Client license: MIT
- Data license: CDLA Permissive 2.0, subject to Overture's per-theme source attribution
...
The helper requests Places for a validated maximum-25-km area and streams GeoJSONSeq into bounded normalized NDJSON. Stored snapshots contain only identity/version, display name, bounded address/contact fields, `basic_category`, `taxonomy`, confidence and operating status. They never contain arbitrary provider payloads or photos.
...
Attribution: `© Overture Maps Foundation, available under CDLA Permissive 2.0`. Review the release's source attribution before a production enablement.
