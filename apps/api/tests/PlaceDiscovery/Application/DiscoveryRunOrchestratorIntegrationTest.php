...
final class DiscoveryRunOrchestratorIntegrationTest extends KernelTestCase
{
    ...
    private function runs(PlaceDiscoveryProvider $provider): DiscoveryRunOrchestrator
    {
        ...
        return new DiscoveryRunOrchestrator($this->connection, $provider, $normalizer, $profile, $service, $locks, $bus, new NullLogger());
    }

    private function cleanup(): void
    {
        $this->connection->executeStatement("DELETE FROM place_candidate_audit_events WHERE candidate_id IN (SELECT id FROM place_candidates WHERE external_id IN ('redelivery','retry','stale','lock'))");
        $this->connection->executeStatement("DELETE FROM place_candidates WHERE external_id IN ('redelivery','retry','stale','lock')");
...
final class DeterministicProvider implements PlaceDiscoveryProvider
{
    ...
    public function __construct(private readonly bool $failFirst, private readonly string $externalId, private readonly bool $alwaysFail = false)
    {
        $this->release = '2099-01-0'.match ($externalId) {
            'redelivery' => '1', 'retry' => '2', 'stale' => '3', default => '4',
        }.'.0';
    }
    ...
    public function streamPlaces(DiscoveryArea $area, string $profile, string $release, int $limit): iterable
    {
        ...
        yield new ProviderPlace($this->externalId, $release, '1', 'Family playground '.$this->externalId, 50.0413, 21.999, 'Rynek 1', '35-001', 'Rzeszów', 'PL', null, null, ['playground'], 'playground', 0.95, 'open', ['id' => $this->externalId], [new ProviderSourceRecord('properties', 'Overture', 'CDLA-Permissive-2.0')]);
    }
}
